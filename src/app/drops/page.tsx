...
export default function DropsPage() {
  const live = weeklyDrops.filter(d => d.status === 'live')
  const upcoming = weeklyDrops.filter(d => d.status === 'upcoming')
  const completed = weeklyDrops.filter(d => d.status === 'completed')

  function DropCard({ drop }: { drop: typeof weeklyDrops[0] }) {
    const progress = mockProgress.find(p => p.drop_id === drop.id)
    const isLive = drop.status === 'live'

    return (
      <Link href={`/drops/${drop.slug}`} className={`group block rounded-2xl p-5 glass hover:bg-white/[0.04] transition-all duration-200 ${isLive ? 'glow-green' : ''}`}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <Badge variant="outline" className={`text-[10px] px-1.5 py-0 rounded-md ${DROP_STATUS_COLORS[drop.status]}`}>
            {isLive && <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse mr-1" />}
            {drop.status}
          </Badge>
          {!drop.is_free && (
        ...
        <div className="flex items-center gap-3 text-[11px] text-neutral-500">
          <span className="flex items-center gap-1">
            <Play className="w-3 h-3" /> {drop.duration_minutes} min
          </span>
          ...
          {drop.prize_amount > 0 && (
            <span className="flex items-center gap-1 text-amber-400">
              <Trophy className="w-3 h-3" /> ${drop.prize_amount}
            </span>
          )}
          {drop.status !== 'upcoming' && (
            <span className="ml-auto">{drop.submissions_count} builds</span>
          )}
        </div>

        {progress && (
          <div className="flex items-center gap-3 mt-4 pt-3 border-t border-white/[0.06]">
            <span className={`flex items-center gap-1 text-[11px] ${progress.watched ? 'text-green-400' : 'text-neutral-600'}`}>
              <CheckCircle2 className="w-3 h-3" /> Watched
            </span>
            <span className={`flex items-center gap-1 text-[11px] ${progress.submitted ? 'text-green-400' : 'text-neutral-600'}`}>
  ...
  return (
    <AppLayout>
      ...
      {live.length > 0 && (
        <div className="mb-10">
          <h2 className="text-[12px] font-semibold text-green-400 mb-3 flex items-center gap-2 uppercase tracking-wider">
            <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" /> Live Now
          </h2>
          <div className="grid md:grid-cols-2 gap-3">
            {live.map(d => <DropCard key={d.id} drop={d} />)}
          </div>
        </div>
      )}
      ...
      {completed.length > 0 && (
        <div>
          ...
          <div className="grid md:grid-cols-2 gap-3">
            {completed.map(d => <DropCard key={d.id} drop={d} />)}
          </div>
        </div>
      )}
    </AppLayout>
