...
import { weeklyDrops, mockLeaderboard, mockPosts, mockProfile, mockProgress } from '@/lib/mock-data'
import { DIFFICULTY_COLORS } from '@/lib/constants'
import { formatDistanceToNow } from 'date-fns'

export default function DashboardPage() {
  const currentDrop = weeklyDrops.find(d => d.status === 'live')
  const completedDrops = weeklyDrops.filter(d => d.status === 'completed')
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { label: 'Completed', value: `${completedDrops.length}/${weeklyDrops.length}`, icon: CheckCircle2, color: 'text-green-400' },
          ...
          <div className="rounded-2xl p-6 relative overflow-hidden glass-strong glow-green">
            ...
            <div className="relative">
              ...
              <div className="flex items-center gap-3 mb-5">
                <Badge variant="outline" className={`text-[11px] px-2 py-0.5 rounded-md ${DIFFICULTY_COLORS[currentDrop.difficulty]}`}>
                  {currentDrop.difficulty}
                </Badge>
                <span className="flex items-center gap-1 text-[12px] text-neutral-500">
                  <Clock className="w-3 h-3" /> {currentDrop.duration_minutes} min
                </span>
                {currentDrop.prize_amount > 0 && (
                  <span className="flex items-center gap-1 text-[12px] text-amber-400">
                    <Trophy className="w-3 h-3" /> ${currentDrop.prize_amount}
                  </span>
                )}
              </div>
              ...
              <Button size="sm" asChild className="bg-white text-black hover:bg-neutral-100 h-9 px-5 text-[13px] font-semibold rounded-xl">
                <Link href={`/drops/${currentDrop.slug}`}>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/community" className="text-[12px] text-neutral-500 hover:text-white transition-colors">View all</Link>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl p-5 glass">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/leaderboard" className="text-[11px] text-neutral-500 hover:text-white transition-colors">View all</Link>
              </div>
