...
import type { Community, Profile, LeaderboardEntry } from '@/types/database'
...
export function LeaderboardClient({ leaderboard, profile }: LeaderboardClientProps) {
  const userLevel = getBuilderLevel(profile.total_points)
  ...
  return (
    <AppLayout>
      ...
      <div className="mb-8 max-w-3xl">
        <button className="text-[11px] text-zinc-600 uppercase tracking-wider font-medium mb-3 flex items-center gap-1.5 hover:text-zinc-400 transition-colors cursor-default">
          ...
          {leaderboard.map(entry => {
            ...
            return (
              <div key={entry.id} className={`grid grid-cols-[2.5rem_1fr_auto_4.5rem] md:grid-cols-[3rem_1fr_auto_5rem] gap-3 px-4 md:px-5 py-3.5 items-center border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02] transition-colors ${isTop3 ? 'bg-white/[0.01]' : ''}`}>
                <span className={`text-[13px] font-bold ${isTop3 ? PODIUM_COLORS[entry.rank - 1].iconColor : 'text-zinc-600'}`}>{entry.rank}</span>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <div className="flex-1 h-1 rounded-full bg-white/[0.06] overflow-hidden max-w-[120px]">
                        <div
                          className={`h-full rounded-full transition-all duration-500 ${level.level >= 6 ? 'bg-gradient-to-r from-amber-500 to-orange-500' : 'bg-blue-500'}`}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-blue-500/10 border border-blue-500/20 flex items-center justify-center">
                <span className="text-[13px] font-medium text-blue-300">{profile.full_name[0]}</span>
              </div>
              <div>
                <p className="text-[13px] font-semibold">Your Progress</p>
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className={`inline-flex items-center gap-1 text-[11px] font-medium ${userLevel.color}`}>
                    {userLevel.emoji} {userLevel.name}
                  </span>
                  {userLevel.nextLevel && (
                    <>
                      <ChevronUp className="w-3 h-3 text-zinc-600" />
                      <span className="text-[10px] text-zinc-600">{userLevel.nextLevel.minPoints - profile.total_points} pts to {userLevel.nextLevel.name}</span>
                    </>
                  )}
                </div>
              </div>
            ...
            <div className="text-right">
              <p className="text-[20px] font-bold text-white">{profile.total_points}</p>
              <p className="text-[10px] text-zinc-600">total pts</p>
            </div>
          </div>
          <div className="mt-3 h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
            <div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-sky-400 transition-all duration-500" style={{ width: `${userLevel.progressToNext}%` }} />
          </div>
        </div>
      </div>
    </AppLayout>
