      ...
      <Link href={`${base}/drops/${drop.slug}`} className={`group block rounded-2xl p-5 glass hover:bg-white/[0.04] transition-all duration-200 ${isLive ? 'glow-blue' : ''}`}>
        ...
        {progress && (
          <div className="flex items-center gap-3 mt-4 pt-3 border-t border-white/[0.06]">
            <span className={`flex items-center gap-1 text-[11px] ${progress.watched ? 'text-blue-400' : 'text-zinc-600'}`}><CheckCircle2 className="w-3 h-3" /> Watched</span>
            <span className={`flex items-center gap-1 text-[11px] ${progress.submitted ? 'text-blue-400' : 'text-zinc-600'}`}><CheckCircle2 className="w-3 h-3" /> Submitted</span>
          </div>
        )}
      </Link>
    )
