...
export default function DropDetailPage() {
  ...
  return (
    <AppLayout>
      <Link href={`${base}/drops`} className="inline-flex items-center gap-1.5 text-[13px] text-neutral-500 hover:text-white mb-8 transition-colors">
      ...
      <div className="max-w-3xl">
        <div className="mb-8">
          ...
          <div className="flex items-center gap-4 mt-3">
            <span className="flex items-center gap-1.5 text-[12px] text-neutral-500"><Clock className="w-3 h-3" /> {drop.duration_minutes} min</span>
        ...
        {/* Video */}
        <div className="mb-10">
          <h2 className="text-[13px] font-semibold mb-4 flex items-center gap-2 uppercase tracking-wider text-neutral-400"><Play className="w-4 h-4 text-blue-400" /> The Lesson</h2>
          {isUpcoming && !drop.video_url ? (
            <div className="aspect-video rounded-2xl bg-white/[0.02] border border-white/[0.06] flex items-center justify-center">
              <div className="text-center"><div className="w-14 h-14 rounded-2xl bg-white/[0.04] flex items-center justify-center mx-auto mb-3"><Play className="w-6 h-6 text-neutral-600" /></div><p className="text-[13px] text-neutral-600">Video drops soon</p></div>
            </div>
          ) : drop.video_url ? (
            <>
        ...
        <div className={`mb-10 transition-opacity duration-300 ${!challengeUnlocked ? 'opacity-30 pointer-events-none' : ''}`}>
          ...
          <div className="grid md:grid-cols-2 gap-3 mb-4">
            ...
            <div className="rounded-2xl p-5 glass">
              ...
              <ul className="space-y-2">{drop.challenge_rules.map((r, i) => <li key={i} className="flex items-start gap-2 text-[13px] text-neutral-400"><AlertCircle className="w-3.5 h-3.5 text-neutral-600 shrink-0 mt-0.5" /> {r}</li>)}</ul>
            </div>
          </div>
          {drop.status === 'live' && <p className="text-[12px] text-neutral-500">Deadline: {formatDistanceToNow(new Date(drop.challenge_deadline))} left</p>}
        </div>
