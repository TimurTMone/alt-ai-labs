...
export default function AdminPage() {
  ...
  const stats = [
    ...
    { label: 'Drops', value: String(drops.length), icon: Play, color: 'text-blue-400' },
            ...
            <div className="space-y-1">
              {drops.map(drop => (
                <div key={drop.id} className="flex items-center justify-between py-2.5 border-b border-white/[0.04] last:border-0">
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${drop.status === 'live' ? 'bg-blue-500/10 text-blue-400' : drop.status === 'upcoming' ? 'bg-blue-500/10 text-blue-400' : 'bg-white/[0.04] text-neutral-600'}`}>{drop.status}</span>
                </div>
              ))}
            </div>
          </div>
