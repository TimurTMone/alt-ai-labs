              ...
              {completedDrops.slice(0, 4).map(drop => {
                ...
                return (
                  <Link key={drop.id} href={`${base}/drops/${drop.slug}`} className="rounded-xl p-3.5 hover:bg-white/[0.03] transition-colors flex items-center gap-3 group">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${dropProgress?.submitted ? 'bg-blue-500/10' : 'bg-white/[0.04]'}`}>
                      {dropProgress?.submitted ? <CheckCircle2 className="w-5 h-5 text-blue-400" /> : <Play className="w-5 h-5 text-zinc-600" />}
                    </div>
                    <div className="min-w-0 flex-1">
