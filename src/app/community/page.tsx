...
export default function CommunityPage() {
  ...
  const regular = mockPosts.filter(p => !p.is_pinned)
    ...
    <AppLayout>
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-1">
        ...
        {POST_CATEGORIES.map(cat => (
          <button key={cat.value} className="text-[12px] px-3.5 py-1.5 rounded-full border border-white/[0.08] text-neutral-500 hover:text-white hover:border-white/[0.15] transition-colors shrink-0">
            {cat.label}
          </button>
        ))}
      </div>
