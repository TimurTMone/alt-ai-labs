...
import { LayoutDashboard, Play, MessageSquare, Medal, Users, User, Shield, LogOut } from 'lucide-react'
    ...
    <aside className={cn('flex flex-col w-[260px] border-r border-white/[0.06] bg-[#09090b] h-screen sticky top-0', className)}>
      <div className="px-5 h-16 flex items-center border-b border-white/[0.06]">
        <Link href="/dashboard" className="flex items-center gap-2.5">
          ...
          <span className="font-semibold text-[15px] text-white tracking-tight">Alt AI Labs</span>
        </Link>
      </div>
      ...
      <div className="px-3 py-4 space-y-0.5 border-t border-white/[0.06]">
        {isAdmin && (
          <Link href="/admin" className={cn('flex items-center gap-3 px-3 py-2 rounded-lg text-[13px] font-medium transition-all duration-150', pathname.startsWith('/admin') ? 'bg-white/[0.08] text-white' : 'text-neutral-400 hover:text-neutral-200 hover:bg-white/[0.04]')}>
